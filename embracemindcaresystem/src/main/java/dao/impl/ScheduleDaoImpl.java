...
public class ScheduleDaoImpl implements ScheduleDao {
    ...
    @Override
    public List<Schedule> findAllForAdmin() {
        List<Schedule> list = new ArrayList<>();
        String sql = BASE_SELECT
                + "ORDER BY s.appointment_date, FIELD(s.session, '上午', '下午', '夜診'), s.start_time, s.id";
    ...
    public void update(Schedule schedule) throws Exception {
        String sql = "UPDATE schedules "
                + "SET appointment_date = ?, session = ?, appointment_type = ?, location = ?, "
        ...
        try (Connection conn = DbConnection.getConnection();
            ...
            ps.setDate(1, java.sql.Date.valueOf(schedule.getAppointmentDate()));
            ps.setString(2, schedule.getSession());
            ps.setString(3, schedule.getAppointmentType());
            ...
            ps.setInt(7, schedule.getQuota());
            ps.setInt(8, schedule.isActive() ? 1 : 0);
